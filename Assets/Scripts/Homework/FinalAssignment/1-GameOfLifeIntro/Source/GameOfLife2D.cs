...
namespace Assignment
{
    ...
    public class GameOfLife2D
    {
        ...
        ///
        /// 
        private void Step2333(int i, int j)
        {
        ...
        private void Step2323(int i, int j)
        {
            int state = _currentState[i, j];
            int sum = GetNeighborSum(i, j);

            if (state == 0)
                _nextState[i, j] = (sum >= 2 && sum <= 3) ? 1 : 0; // dead rule
            else
                _nextState[i, j] = (sum < 2 || sum > 3) ? 0 : 1; // alive rule
        }


        private void Step2525(int i, int j)
        {
            int state = _currentState[i, j];
            int sum = GetNeighborSum(i, j);

            if (state == 0)
                _nextState[i, j] = (sum >= 2 && sum <= 5) ? 1 : 0; // dead rule
            else
                _nextState[i, j] = (sum < 2 || sum > 3) ? 0 : 1; // alive rule
        }
